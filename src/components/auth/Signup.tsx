...
const Signup = () => {
  ...
  const {
    ...
    isVerifyingOtp,
    isUserLoggedIn
  } = useSelector((state: RootState) => state.auth.signup);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      await verifyOtp({ otp, name, idToken }).unwrap();
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 to-black p-4">
        <div className="mb-10">
          <img src={Logo} alt="Nava Cloth Logo" className="h-40 w-auto" />
        </div>
        <div className="bg-gray-800 text-white rounded-2xl shadow-2xl p-8 w-full max-w-sm">
          ...
          {step === "verifyOtp" && (
            <>
              ...
              <p className="text-center text-gray-400 mb-6">
                Enter your name and the OTP sent to your phone
              </p>
              <input
                type="text"
